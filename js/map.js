/* ============================================
   HAWDAJ GAME — Map Game Logic (v2 dual-mode)
   ============================================ */

const MapGame = (() => {
    // ── State ─────────────────────────────────────────────────────────────
    let completedRegions = [];
    let completedCities  = [];
    let selectedRegionGroup = null;
    let activeMapMode = 'regions'; // 'cities' | 'regions'

    // For clothing carousel
    let carouselItems  = [];
    let carouselIndex  = 0;

    // For TTS / audio playback
    let currentAudio = null;
    let isSpeaking   = false;

    // Track last opened item ID (city or geo-region)
    let lastClickedId = null;

    // Mapping the 13 administrative SVG paths → 5 geographic regions
    const REGION_MAPPING = {
        riyadh: 'central', qassim: 'central', hail: 'central',
        eastern: 'eastern',
        tabuk: 'northern', jawf: 'northern', northern: 'northern',
        makkah: 'western', madinah: 'western',
        asir: 'southern', jazan: 'southern', najran: 'southern', bahah: 'southern'
    };

    // Fact emojis for visual cards (cycle through these)
    const FACT_EMOJIS = ['🏙️','🌟','🕌','🌄','⭐','🌺','🏔️','🗺️','🎉','✨','🐪','🌿'];

    // Fashion category labels
    const FASHION_LABELS = {
        boy: 'ولد 👦', girl: 'بنت 👧',
        girl1: 'بنت 👧', girl2: 'بنت 2 👧',
        man: 'رجل 👨', man2: 'رجل 2 👨', man3: 'رجل 3 👨',
        woman: 'امرأة 👩', women: 'امرأة 👩', women2: 'امرأة 2 👩'
    };

    // ── Init ──────────────────────────────────────────────────────────────
    function init() {
        const data = HawdajData.get();
        completedRegions = data.player.completedRegions || [];
        completedCities  = data.player.completedCities  || [];

        selectedRegionGroup = null;

        // Attach toggle switch listeners
        const btnCities  = document.getElementById('btn-mode-cities');
        const btnRegions = document.getElementById('btn-mode-regions');
        if (btnCities) {
            btnCities.onclick = () => { if (activeMapMode !== 'cities')  { activeMapMode = 'cities';  syncModeButtons(); renderMap(); updateProgress(); } };
        }
        if (btnRegions) {
            btnRegions.onclick = () => { if (activeMapMode !== 'regions') { activeMapMode = 'regions'; syncModeButtons(); renderMap(); updateProgress(); } };
        }

        syncModeButtons();
        renderMap();
        updateProgress();

        // Background click deselects
        const mapScreen = document.getElementById('map-game');
        if (mapScreen) {
            mapScreen.removeEventListener('click', handleBackgroundClick);
            mapScreen.addEventListener('click', handleBackgroundClick);
        }

        // Characters
        const az = document.getElementById('map-char-azzam');
        const jd = document.getElementById('map-char-jadel');
        if (az) { az.className = 'game-char-img azzam'; az.src = HawdajData.getCharacterPose('azzam','happy') || 'assets/char/azzam.jpeg'; }
        if (jd) { jd.className = 'game-char-img jadel'; jd.src = HawdajData.getCharacterPose('jadel','talking') || 'assets/char/El jadel.jpeg'; }

        // Intro dialogue
        const introAudio = HawdajAudio.getRandomAudioPath('jadel','intro_map');
        HawdajDialogue.showIntro(
            'أهلاً بك في خريطة المملكة التفاعلية! اختر خريطة المدن لتتعلم معلومات رائعة عن كل منطقة، أو خريطة المناطق لتستكشف الأزياء التراثية الجميلة!',
            introAudio,
            () => {
                if (az) az.src = HawdajData.getCharacterPose('azzam','thinking') || 'assets/char/azzam.jpeg';
                if (jd) jd.src = HawdajData.getCharacterPose('jadel','thinking') || 'assets/char/El jadel.jpeg';
            }
        );
    }

    function syncModeButtons() {
        const btnC = document.getElementById('btn-mode-cities');
        const btnR = document.getElementById('btn-mode-regions');
        if (btnC) btnC.classList.toggle('active', activeMapMode === 'cities');
        if (btnR) btnR.classList.toggle('active', activeMapMode === 'regions');
    }

    function handleBackgroundClick(e) {
        if (e.target.tagName !== 'path' &&
            !e.target.closest('.map-selection-card') &&
            !e.target.closest('.map-compass-btn')) {
            deselectRegion();
        }
    }

    // ── Render Map ────────────────────────────────────────────────────────
    function renderMap() {
        const container = document.getElementById('map-container');
        if (!container) return;

        container.innerHTML = `
    <svg viewBox="0 0 730 600" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:auto;">
      <path id="region-riyadh" d="m 456.82718,487.03041 -10.13,2.63 -16.39,2.39 -58.85,5.04 -8.85,-0.7 -3.24,-0.72 -5.22,-2.39 -8.51,-5.59 0,0 -13.54,-12.09 -3.91,-6.36 -2.75,-2.96 -0.22,-1.6 1.23,-6.11 3.34,-6.09 0.08,-2.96 -1.06,-1.73 -3.6,-3.6 -3.07,-4.93 -5.76,-5.01 0,0 -4.34,-7.06 -4.77,-6.36 -0.58,-1.47 1.76,-8.48 -0.54,-6.95 2.33,-3.42 0.68,-2.34 -1.79,-13.1 0.96,-6.16 -0.31,-1.57 -6.45,-1.41 -5.65,1.47 -3.7,-1.28 -1.26,-2.3 0.28,-5.34 0.86,-2.93 -1.84,-1.88 -3.09,-0.85 -14.1,-0.63 -3.01,-1.63 -5.15,-11.26 -0.91,-5.64 -2.1,-4.72 -1.13,-4.86 -4.16,-3.3 -0.36,-3.13 0.44,-3.64 0,0 -0.04,-9.03 1.41,-4.88 -0.94,-3.6 2.95,-4.25 0.4,-3.14 0,0 3.04,-5.08 2.17,-1.58 1.5,0.12 5.17,2.55 3.87,0.81 5.73,0.5 5.67,-0.89 5.69,0.87 2.43,-1.79 0.85,-2.18 -1.91,-3.83 0.15,-2.3 5.42,-6.69 3.27,-2.52 12.88,-3.15 2.84,-2.81 3.09,-1.74 2.28,-4.47 2.02,-1.97 3.13,-0.82 8.3,1.06 7.2,-0.68 5.53,0.22 0.84,-3.9 -0.31,-3.16 -2.52,-4.86 1.08,-2.91 -0.14,-0.86 -6.69,-10.16 -3.6,-3.57 -0.49,-1.52 0.84,-5.74 -0.43,-5.12 4.01,-1.1 4.5,-5.87 2.46,-2.11 4.18,-1.84 0.62,-1.48 -0.15,-2.5 -1.34,-2.5 -3.6,-2.99 -2.35,-0.93 0,0 -0.68,-1.88 0.06,-1.59 0.99,-3.1 2.42,-3.32 3.15,-1.64 0,0 1.94,2.12 1.51,0.52 1.8,-0.86 1.3,-2.23 0,0 3.32,2.32 2.12,2.21 1.4,5.11 1.24,1.53 10.41,6.08 5.75,1.06 3.46,4.22 4.06,3.38 2.34,0.63 5.64,0.11 3.01,0.81 9.86,6.94 2.49,1.19 2.36,0.05 4.41,-1.01 1.55,0.22 6.45,6.08 1.47,2.52 8.88,3.16 3.86,2.08 1.09,2.56 0.43,4.96 0.38,22.63 -0.63,15.07 0.35,7.41 0.68,3.7 1.12,1.79 2.28,1.8 9.44,6.3 9.19,9.16 1.68,2.06 2.95,5.78 1.36,6.89 -0.02,7.75 z" data-region="riyadh" />
      <path id="region-makkah" d="m 143.70718,343.80041 0,0 0.12,-0.02 -0.12,0.02 z m 114.74,-19.79 -0.44,3.64 0.36,3.13 4.15,3.3 1.13,4.86 2.11,4.72 0.91,5.64 5.15,11.26 3.01,1.63 14.1,0.63 3.09,0.85 1.84,1.88 -0.85,2.93 -0.29,5.33 1.26,2.31 3.7,1.28 5.65,-1.47 6.45,1.41 0.31,1.57 -0.96,6.17 1.79,13.1 -0.68,2.34 -2.33,3.43 0.55,6.94 -1.76,8.48 0.58,1.47 4.77,6.36 4.34,7.06 0,0 -4.02,1.47 -5.71,3.77 -5.22,-1.3 -2.26,0.25 -3.16,1.52 -4.55,0.28 -4.26,3.81 -6.41,3.06 -10.11,9.24 -2.31,1.14 -2.29,-0.24 -1.4,-1.02 -4.67,-9.07 0,0 -1.98,-1.93 -1.48,-0.22 -5.42,2.76 -4.16,-0.2 -4.61,1.77 -1.91,-1.46 -1.28,-3.77 -1.22,-1.45 -1.43,-0.61 -2.26,0.32 -1.36,1.41 -0.7,2.12 1.11,4.54 -0.12,2.36 -2.69,4.63 -1.37,3.56 -7.73,1.86 -4,4.1 0.17,1.42 0.67,0.7 3.66,1.09 0.97,1.53 -0.31,5.28 0.91,3.89 -0.27,4.44 1.3,1.54 3.69,2.13 2.06,3.06 1.59,1.05 2.59,0 2.21,-1.08 2.58,-8.98 2.4,-4.46 1.52,-0.59 4.61,-0.04 0.79,-0.5 0,0 1.51,-0.74 2.29,0.13 1.85,1.88 0.21,1.52 -1.31,6.92 2.19,4.44 -1.27,4.6 -0.76,1.42 -1.52,0.75 -4.74,-0.43 -2.24,0.37 -3.88,2.82 -0.75,0.06 -1.97,-1.87 -0.49,10 0.61,2.97 1.02,1.48 1.44,0.48 6.71,0.21 2.11,1.21 2.98,9.14 -1.5,6.41 0,0 -1.01,1.94 -3.55,2.05 -1.97,3.15 0,0 -0.4,-1.87 -0.27,-0.33 -0.95,-1.23 0.03,-0.49 -0.32,-0.57 -0.37,-0.37 -0.51,0.76 -0.36,-0.35 0.36,-1.15 -0.46,-0.24 -0.48,-0.65 0.15,-0.87 -0.73,-0.87 -0.54,-2.04 0.05,-0.8 -0.17,-1.41 -0.82,-0.8 0.15,-1.21 -1.41,-1.06 -0.63,-1.3 -2.26,-0.09 -1.06,-0.83 -0.19,-1.62 -1.74,-2.17 1.24,-4.89 -1.56,-0.4 -1.15,-0.92 -1.2,-2.73 1.29,-3 -0.2,-1.69 -3.05,-1.3 -1.11,-2.63 0.38,-1.51 -0.45,-1.44 -0.82,-0.58 -0.56,-1.36 -1.58,-0.81 0.3,-0.43 -0.48,-2.03 0.5,-2.77 -1.22,-1.31 -2.27,-0.56 -2.79,-2.11 -0.28,-1.02 1.04,0.93 0.34,-2.1 -1.13,-3.14 -1.43,-1.64 -2.06,0.57 -1.01,-0.32 0.18,0.78 1.11,0.52 -0.3,0.28 -1.35,-0.84 -0.43,-1.5 -1.42,-0.71 -0.3,-2 -1.3,-0.58 0.42,-1.26 -0.41,-1.12 -5.69,-3.91 -2.98,-0.95 -1.24,-2.03 -0.65,-0.05 0.14,-1.25 -2.29,0.07 -1.45,-1.52 -0.51,-1.48 -1.65,-0.59 -1.89,0.26 0.06,0.51 -0.75,0.13 -2.1,-1.56 -1.14,0.2 1.53,0.77 0.28,0.74 -2.67,-1.27 -5.15,-3.89 -1.76,-1.95 0.58,-0.54 -1.07,-0.75 0.18,-1.07 -0.96,0.31 -0.7,-0.68 -1.01,-1.32 0.23,-0.37 1,0.68 -0.4,-1.17 -0.99,0.09 -1.48,-1.98 -1.64,-3.02 0.52,-0.77 -0.17,-0.74 -1.45,-0.9 -0.15,0.64 0.6,0.97 -0.58,-0.22 -0.8,-2.17 0.89,0.43 0.34,-0.2 -0.53,-0.43 0.29,-0.25 -0.67,-0.12 -0.55,0.19 -2.05,-1.72 0.25,-1.31 -0.84,-0.77 -1.29,-0.07 -1.24,-1.94 0.16,-1.2 -1.32,-2.35 -1.66,-1 -0.89,-4.24 -0.83,-0.7 -1.27,-2.5 0.11,-0.78 0.87,-0.39 1.29,-0.9 0.7,-1.87 -1.22,-1.02 0.13,-0.82 -0.26,-0.74 0.9,-0.1 -0.92,-1.34 0.25,-0.39 -0.59,0 -0.24,0.29 -0.78,-0.67 -0.93,-6.07 -1.43,-3.12 -3.24,-4.23 -0.38,-1.86 0.39,-0.47 -0.51,-1.44 0.59,-0.78 1.37,-0.28 -1.02,1.88 0.52,0.64 0.46,-0.88 0.79,0.17 0.2,-2.75 1.22,0.1 -0.24,-0.86 0.56,-2.21 -0.34,-0.69 -0.56,2.85 -0.58,-0.26 0.8,-1.95 -0.32,-1.47 2.25,-4.19 -0.02,-1.7 0.44,-0.72 0.84,-0.18 0.32,-1.38 -1.17,-0.01 -0.15,0.92 -0.33,-0.19 -0.72,-0.57 0.48,-2.14 -0.1,-3.6 -1.88,-3.15 -0.39,-1.76 -1.17,-1.43 0.89,-0.9 -0.93,-0.07 -0.43,0.31 -4.27,-7.13 1.3,0.49 1.37,2.24 -0.19,0.47 1.19,0.21 0.51,-0.72 -1.58,-2.25 -0.05,-1.49 -2.29,0 0.32,-0.53 -0.89,0.1 -0.26,0.35 -1.44,-2.19 0.74,-1.65 -0.73,-3.8 -0.25,-0.76 -0.68,0.68 -1.68,-2.45 -0.09,-1.34 -0.33,-0.39 -0.23,-0.15 -0.98,-2.23 -0.07,-2.6 0,0 5.12,-0.4 3.08,-0.92 0.82,0.34 2.13,3.42 2.42,1.85 3.42,4.08 1.68,1.3 1.55,0.23 4.88,-1.11 1.58,0.37 0.43,3.86 0.7,0.58 10.44,-3.25 4.76,-0.31 1.07,1.39 0.28,3.25 -2.65,6.93 0.06,0.86 0.59,0.74 3.98,1.18 2.09,1.91 0.45,2.31 -0.83,3.87 0.65,1.58 4.76,0.56 2.33,-1.12 4.63,-4.1 6.79,2.21 1.62,-0.02 4.47,-6.66 9.34,-7.12 5.12,-5.32 2.45,-1.77 5.67,-2.31 0.56,-0.76 -0.4,-1.57 -5.35,-2.94 -0.76,-0.66 -0.2,-1.49 3.59,-2.87 11.75,-5.83 1.06,-1.53 0.69,-2.21 -0.16,-3.22 -1.11,-2.52 0.58,-2.32 3.24,-1.03 5.03,0.65 z" data-region="makkah" />
      <path id="region-madinah" d="m 130.84718,324.65041 -0.9,-0.61 -0.33,0.61 -0.7,-0.29 -0.33,-0.44 -1.1,-1 0.38,-0.9 -1,0.41 -1.03,-1.53 -1.82,-1.19 0.16,-0.31 -0.62,-0.34 -1.13,0.46 -3.64,-2.84 0.62,-0.51 -0.59,-0.29 0.58,-1.68 -1.39,0.76 0.45,0.38 -0.44,1.27 -1.68,-0.19 -2.75,-2.06 -1.19,-2.11 -0.79,0.16 -0.38,-0.71 -0.69,-0.14 -0.09,-0.83 -1.55,0.6 -1.04,-0.36 -0.08,0.81 0.54,0.16 -0.37,0.91 -1.51,0.05 -0.66,-0.98 -1.17,0.09 -1.82,-2.71 -1.49,-1.07 0.31,-1.82 0.67,0.46 0.26,-1.12 -1.18,-1.47 -0.42,-1.45 -1.040004,-0.88 0,0 0.550004,-3.24 0.95,-1.47 5.65,-0.48 0,0 2.36,-0.91 0,0 0.79,-0.71 0,0 1.17,-1.81 0,0 0.55,-0.71 0,0 1.55,-0.41 2.31,0.45 0,0 2.58,-0.03 1.61,-0.83 0.92,-1.54 0.22,-1.56 0,0 -0.11,-3.18 0,0 -0.22,-10.27 0,0 0.84,-4.77 0,0 0.1,-0.74 0,0 0.25,-2.3 0,0 -1.12,-1.4 0,0 -3.17,-0.13 0,0 -0.74,-0.31 0,0 -1.79,-6.32 0,0 -1.8,-3.88 -3.72,-3.33 0,0 -2.17,-1.85 -0.67,-1.64 0,0 -0.54,-2.27 -0.73,-0.61 0,0 -2.23,-1.15 0,0 -4.010004,-2.52 -1.53,0.17 0,0 -1.95,1.78 -0.28,-1.56 0,0 -0.81,-3.12 -1.69,-3.17 0,0 -0.67,-1.68 0,0 0.16,-2.33 0,0 3.15,-5.54 0.26,-1.76 0,0 -0.36,-1.18 0,0 -1.54,-0.95 0,0 -4.78,-1.35 0,0 -1.6,-1.31 0,0 -1,-1.58 -0.69,-2.63 0.81,-8.93 0,0 -0.99,-4.2 0,0 -1.43,-1.38 0,0 -1.54,-0.18 0,0 -3.12,0.69 0,0 -0.81,-0.48 -0.09,-0.75 1,-1.47 0,0 3.12,-0.68 0,0 1.62,-0.43 0,0 1.67,-0.81 0,0 1.59,-0.57 0,0 3.22,-1.5 0,0 1.64,-1.06 0,0 0.76,-0.34 0,0 1.55,-0.39 0,0 3.96,1.66 0,0 2.360004,1.74 0,0 2.42,0.31 1.73,-0.42 5.85,-3.33 0,0 1.55,-0.46 0,0 0.78,0.31 0,0 1.11,1.52 0,0 0.8,3.16 0,0 0.56,1.68 0,0 2.45,4.27 0,0 1.09,1.55 0,0 0.81,0.56 0,0 1.52,0.26 0,0 5.38,0.24 1,1.24 0,0 -0.2,1.59 0,0 0.15,0.79 1.47,1.04 0,0 2.27,1.42 0,0 0.15,0.75 0,0 -0.43,2.4 0.19,0.75 0.74,0.34 0,0 0.75,-0.01 1.57,-1.19 0,0 1.54,-1.37 0,0 1.69,-1.29 1.65,-0.33 6.16,2.51 0.8,-0.64 -0.52,-1.5 0,0 -0.31,-1.5 0.42,-0.73 0.75,-0.04 0,0 1.28,1.18 0.96,3.08 0,0 2.36,0.83 0,0 2.36,0.58 2.27,1.43 0,0 1.52,1.23 0,0 2.42,1.83 2.26,0.41 0,0 6.57,-0.52 0,0 10.45,-2.02 0,0 1.53,-0.07 0,0 0.77,0.43 0,0 1.22,1.43 0,0 0.56,1.01 0,0 0.68,1.56 0.24,2.66 -0.75,2.42 -1.62,2.25 0,1.63 1.18,1.5 0.77,0.24 3.27,-0.84 0.73,0.25 0.36,0.79 -0.11,4.16 -1.9,10.91 2.08,9.8 -0.52,10.14 0.63,1.61 1.3,1.49 1.56,0.71 1.55,0.03 9.37,-3.24 8.06,-0.39 2.41,-1.18 3.87,-4.64 3.27,-1.25 6.68,-0.15 11.8,2.31 0,0 6.92,1.83 1.35,1.16 0.28,1.98 -0.57,11.32 1.74,2.02 5.3,1.39 1.41,1.13 3.15,10.79 4.08,3.38 0,0 -0.4,3.14 -2.95,4.25 0.94,3.6 -1.41,4.88 0.04,9.03 0,0 -5.05,-0.85 -3.23,1.03 -0.58,2.32 1.11,2.52 0.16,3.23 -0.69,2.21 -1.06,1.53 -11.75,5.84 -3.59,2.87 0.2,1.49 0.76,0.66 5.35,2.94 0.4,1.57 -0.56,0.76 -5.67,2.31 -2.45,1.77 -5.12,5.33 -9.34,7.12 -4.47,6.66 -1.62,0.02 -6.79,-2.21 -4.63,4.1 -2.33,1.12 -4.76,-0.56 -0.65,-1.58 0.83,-3.86 -0.45,-2.31 -2.09,-1.91 -3.98,-1.18 -0.59,-0.74 -0.06,-0.86 2.65,-6.92 -0.28,-3.25 -1.07,-1.39 -4.76,0.31 -10.44,3.25 -0.7,-0.58 -0.43,-3.86 -1.58,-0.37 -4.88,1.11 -1.55,-0.23 -1.67,-1.3 -3.42,-4.08 -2.42,-1.84 -2.12,-3.42 -0.82,-0.34 -3.08,0.92 -5.12,0.4 0,0 -0.21,0.03 0,0 -0.12,0.02 0,0 -0.64,-3.24 -1.32,-1.32 0.21,-1.05 -0.97,0.4 -0.65,-0.35 -0.13,0.75 0.84,0.4 -0.37,0.4 -0.86,-0.5 0.1,-1.51 -0.57,-1.35 0.13,-0.52 -0.78,-0.4 -0.7,-0.82 -0.95,-2.79 -0.01,-1.63 -1.02,-1.12 -0.92,-0.09 -0.87,-1.56 -2.41,-1.65 -0.58,-1.03 -0.44,-0.28 z" data-region="madinah" />
      <path id="region-eastern" d="m 545.08718,262.60041 1.22,0.53 0.35,2.07 -0.6,-1.32 -1.01,-0.6 0.04,-0.68 z m -8.35,-39.01 0.87,0.69 -0.05,1.6 -1.18,-1.16 -1.01,-0.12 0.21,-0.65 1.16,-0.36 z m -18.37,-29.14 1.75,1.05 4.11,1.08 -1.87,0.05 -0.52,-0.26 -2.37,0.75 -0.45,-0.6 0.45,-0.25 -0.11,-0.49 -1.17,-0.55 -2.13,2.34 0.34,0.57 -0.4,-0.13 -0.22,-0.78 0.55,-1.55 0.7,-0.87 1.34,-0.36 z m 19.57,33.55 0.38,0.99 0.46,-1.63 0.45,0.11 -0.06,2.39 0.79,0.44 1.19,-1.76 -0.36,-0.33 0.38,-1.81 0.25,0.39 0.12,1.06 0.38,-0.61 0.05,1.18 -0.98,0.58 0.08,1.05 -0.63,0.11 1.65,1.52 0.52,1.62 -0.54,4.1 0.45,0.68 -0.36,0.03 -0.1,1.48 -1,1.13 0.19,-0.78 -0.87,-1.22 -0.16,2.24 0.42,0.52 -0.83,2.12 0.19,1.3 -0.75,0.42 -2.62,-5.96 -0.76,-0.54 -0.52,0.32 -1.15,2.79 0.89,1.29 -0.52,0.67 0.04,3.02 0.61,0.16 0.8,-0.92 0.81,0.74 1.37,0.1 0.5,0.75 -0.15,0.79 0.63,0.57 -0.69,0.16 -0.37,1.04 1.28,2.91 1.81,2.68 1.09,0.66 0.7,2.02 0.75,0.4 -0.23,0.88 -0.41,0.18 -0.4,-1.47 -1.42,-0.33 -1,-1.47 -0.55,0.17 -0.47,-0.58 0.68,1.74 1.09,0.76 0.05,0.56 0.51,-0.72 0.22,0.28 0.24,1.8 1.28,1.16 0.67,1.46 1.26,0.44 0.07,1.1 2.04,2.54 0.4,0.04 -0.05,-1.1 -0.68,-0.91 0.66,-0.53 0.62,1.64 3.23,2.49 1.33,4.04 -0.34,2.41 1.25,2.03 -0.41,1.78 0.83,2.56 -0.27,0.88 0.89,0.45 -0.5,-0.65 0.32,-0.82 2.52,3.53 1.05,3.3 0.95,0.04 0.54,0.69 1.49,6.53 0.64,-1.84 1.58,-0.3 -0.25,0.48 1.08,2.13 3.69,4.38 4.58,0.8 2.45,-0.96 1.63,-1.8 1.16,0.39 1,-0.33 -0.1,0.51 0.6,0.44 0.76,-0.35 -0.66,1.28 0.13,2.42 1.45,-0.87 0.37,-1.39 1.6,-1.3 -0.09,-0.95 0.89,0.53 0.53,-0.43 0.41,1.64 1.03,-0.58 0.76,0.76 -0.62,0.64 -1.21,0.07 -0.54,0.95 -0.44,-0.16 -0.42,1.48 -3.07,2.44 -0.6,3.09 -0.56,0.13 0.13,1.7 2.24,0.42 0.9,-1.18 2.03,-0.18 1.67,0.66 1.51,2.12 1.63,-0.07 0.75,1.18 -0.51,4.23 34.3,41.63 23.79,4.76 59.71,13.76 7.05,-7.46 15.68,25.98 -22.97,73.24 -103.4,36.4 -39.96,7.94 -57.87,6.31 -32.09,15.68 -20.91,25.6 -4.87,12.21 -1.53,0.93 0,0 2.59,-24.45 7.99,-63.49 0,0 19.5,-152.83 0.02,-7.75 -1.36,-6.9 -2.96,-5.78 -1.67,-2.06 -9.19,-9.15 -9.44,-6.3 -2.29,-1.8 -1.12,-1.79 -0.68,-3.7 -0.34,-7.4 0.63,-15.07 -0.38,-22.62 -0.43,-4.96 -1.09,-2.56 -3.86,-2.08 -8.88,-3.17 -1.46,-2.52 -6.45,-6.08 -1.55,-0.22 -4.41,1.01 -2.35,-0.04 -2.5,-1.19 -9.86,-6.94 -3.01,-0.81 -5.64,-0.11 -2.35,-0.63 -4.06,-3.38 -3.46,-4.22 -5.74,-1.06 -10.41,-6.08 -1.23,-1.53 -1.41,-5.11 -2.12,-2.21 -3.32,-2.32 0,0 -1.47,-1.82 -0.38,-1.53 1.28,-4.67 9.15,-12.15 12.66,-23.67 8.26,-8.72 12.45,-4.39 0,0 5.1,0.13 2.64,-1.27 4.09,-3.16 31.35,4.11 0.87,0.67 3.36,5.38 0.61,2.55 0.22,4.61 3.53,4.7 -0.17,0.7 24.98,-0.43 1.18,1.14 1.28,0.47 -0.04,2.25 0.84,0.66 -0.18,0.34 0.85,0.15 -1.01,3.01 0.97,1.98 1.47,1.03 1.7,2.23 -0.69,0.53 -0.18,2.67 0.39,1 2.05,2.38 3.89,1.05 -0.77,0.92 -0.84,-0.08 0.08,1.05 1.59,1.81 1,0.1 0.05,1.35 2.03,1.26 -0.36,2.1 0.46,0.44 -0.08,0.47 -0.9,0.6 0,-0.89 -0.55,-0.04 0.29,-2 -0.72,1.02 -0.29,-1.41 -1.3,4.4 0.64,0.08 0.45,-1.25 0.35,2.38 0.64,-0.95 0.42,0.68 -0.34,1.18 1.07,-0.31 -0.32,0.58 -0.52,0.11 -0.05,0.82 -0.32,0.69 1.12,0.54 0.75,-1.32 0.63,0.19 -0.57,1.04 0.11,1.29 0.81,-0.41 -0.1,-1.04 1.56,-0.79 0.94,0.37 1.08,1.57 2.59,1.16 3.66,-0.06 1.69,0.83 0.61,1.33 1.12,0.63 0.2,1.82 -0.56,-0.15 0.25,-0.69 -0.36,-0.54 -0.71,1.15 -0.54,-0.61 -0.71,0.71 -0.75,-0.72 -0.74,0.47 -0.31,-0.45 -0.69,0.92 -0.82,-0.34 -0.33,0.73 0.41,-0.21 1.54,0.94 0.56,0.71 1.01,-0.24 -0.03,-1.05 1.36,0.77 -0.61,0.76 -0.73,0.07 0.32,0.92 -0.67,1.23 1.72,-1.09 1.62,0.54 -0.01,3.71 0.67,1.13 0.54,0.02 0.42,-0.06 0.53,-0.23 1.14,0.57 -0.48,1.89 -0.65,-0.23 -0.44,0.41 0.51,0.01 -0.2,0.56 0.76,-0.31 0.53,1.03 0.67,-1.73 0.5,0.35 -0.15,1.06 1.2,-0.66 1.46,0.51 -0.89,-1.68 1.27,-1.31 -1.73,-1.58 -0.68,0.08 0.65,-1.04 -0.21,-0.53 0.74,-0.47 0.46,1.97 1.11,0.06 0.23,1.5 -0.74,0.31 0.97,0.96 0.9,-0.7 0.59,-2.69 0.97,-1.31 -0.99,2.95 -0.44,1.56 -0.63,0.96 0.55,1.96 1.17,1.16 2.4,-1.43 1.07,0.41 -0.03,0.09 -0.49,-0.01 -0.09,0.04 -0.27,0.47 -0.27,-0.47 -2.28,1.16 0.96,1.23 1.49,-0.36 -0.16,0.73 -0.56,0.02 0.02,1.19 0.71,0.03 0.24,0.85 0.13,0.2 2.3,1.81 0.37,0.11 1.21,0.73 1.13,0.37 0.79,0.61 2.98,0.3 2.07,2.21 4.49,5.29 0.49,0.45 0.41,0.4 -0.59,0.09 -1.3,-1.73 -1.68,-0.43 -0.98,0.68 -1.23,-1.95 -0.06,-0.88 -0.47,2.24 0.15,0.95 0.81,0.2 -0.35,1.09 0.31,2.66 0.93,2.33 1.41,1.84 0.1,-1.15 0.31,-0.31 z" data-region="eastern" />
      <path id="region-qassim" d="m 238.57718,264.11041 1.2,-1.37 4.95,-0.74 6,-3.57 5.95,-0.77 0.81,-0.65 0.05,-0.82 -2.4,-4.78 0.32,-2.44 1.26,-1.51 0.86,-0.19 3.95,1.42 2.43,0.25 0.47,-0.77 -0.82,-6.33 1.89,-4.8 0.77,-0.41 1.6,0.52 3.32,-0.78 0.87,-4.3 4.75,-4.83 11.89,-9.24 11.31,-5.23 2.76,-5.79 2.72,-2.76 14.5,-7.43 4.15,-1.34 2.37,0.86 4.44,4.49 5.99,4.12 7.47,0.54 5.05,-5.84 0,0 2.35,0.93 3.6,2.99 1.34,2.5 0.15,2.5 -0.62,1.48 -4.18,1.84 -2.46,2.11 -4.5,5.87 -4.01,1.1 0.43,5.12 -0.84,5.74 0.49,1.52 3.6,3.57 6.69,10.16 0.14,0.86 -1.08,2.91 2.52,4.86 0.31,3.16 -0.84,3.9 -5.53,-0.22 -7.2,0.68 -8.3,-1.06 -3.13,0.82 -2.02,1.97 -2.28,4.47 -3.09,1.74 -2.84,2.81 -12.88,3.15 -3.27,2.52 -5.42,6.69 -0.15,2.3 1.91,3.83 -0.85,2.18 -2.43,1.79 -5.69,-0.87 -5.67,0.89 -5.73,-0.5 -3.87,-0.81 -5.17,-2.55 -1.5,-0.12 -2.17,1.58 -3.04,5.08 0,0 -4.08,-3.38 -3.15,-10.8 -1.4,-1.13 -5.31,-1.39 -1.74,-2.01 0.58,-11.32 -0.28,-1.98 -1.35,-1.16 z" data-region="qassim" />
      <path id="region-hail" d="m 183.65718,221.36041 1.99,-2.39 2.34,-7.76 3.13,-3.74 0,0 4.44,-4.13 0.29,-2.41 0,0 -0.36,-0.76 0,0 -2.95,-1.84 0,0 -0.82,-0.36 0,0 -3.73,-2.16 -1.4,-3.91 0,0 -0.14,-0.92 0,0 -0.39,-1.63 0,0 -2.24,-9.37 -1.05,-2.51 0,0 -1.4,-1.64 -2.37,-0.99 0,0 -0.77,-0.46 -1.33,-2.43 -0.75,-0.31 0,0 -2.31,-0.67 0,0 -1.3,-1.59 0,0 -2.94,-2.27 -4.95,-1.3 0,0 -1.54,-0.24 0,0 -4.08,-0.72 0,0 -2.36,-0.52 -1.13,-1.33 0,0 3.69,-4.2 6.27,-3.54 9.93,-4.38 4.29,-0.69 13.07,-7.6 12.9,-5.53 3.04,0.66 0,0 4.38,1.9 2.37,0.37 20.96,-4.87 12.17,3.53 4.76,-1.35 4.45,-0.36 14.47,3.51 2.37,1.99 4.2,5.16 2.52,1.46 3.99,-0.21 4.62,-2.48 2.68,-0.47 0.67,0.8 0.88,6.22 1.42,1.46 0.96,0.31 6.49,-0.31 6,3.12 2.52,-0.84 5.05,-3.38 1.57,-0.21 1.58,1.23 1.08,1.62 5.28,9.66 2.78,7.7 2.02,2.48 2.44,1.44 9.98,1.29 3.28,1.26 1.62,0.95 6.08,5.52 1.03,1.87 0,0 -3.15,1.64 -2.42,3.32 -0.99,3.1 -0.06,1.59 0.68,1.88 0,0 -5.05,5.84 -7.47,-0.54 -5.99,-4.12 -4.44,-4.49 -2.37,-0.86 -4.15,1.34 -14.5,7.43 -2.72,2.76 -2.76,5.79 -11.31,5.23 -11.89,9.24 -4.75,4.83 -0.87,4.3 -3.32,0.78 -1.6,-0.52 -0.77,0.41 -1.89,4.8 0.82,6.33 -0.47,0.77 -2.43,-0.25 -3.95,-1.42 -0.86,0.19 -1.26,1.51 -0.32,2.44 2.4,4.78 -0.05,0.82 -0.81,0.65 -5.95,0.77 -6,3.57 -4.95,0.74 -1.2,1.37 0,0 -11.8,-2.31 -6.69,0.15 -3.27,1.25 -3.87,4.64 -2.41,1.18 -8.06,0.39 -9.37,3.24 -1.55,-0.03 -1.56,-0.7 -1.3,-1.49 -0.63,-1.61 0.51,-10.14 -2.07,-9.81 1.9,-10.9 0.12,-4.16 -0.37,-0.79 -0.73,-0.24 -3.27,0.83 -0.76,-0.24 -1.18,-1.5 0,-1.62 1.62,-2.26 0.74,-2.41 -0.24,-2.66 z" data-region="hail" />
      <path id="region-tabuk" d="m 89.197176,284.95041 1.2,0.84 0,0.66 -1.49,-0.68 0.29,-0.82 z m -7.94,-17.27 0.92,0.6 0.42,-0.21 0.96,2.68 -0.7,0 0.08,-1.17 -2.04,-1.11 0.36,-0.79 z m 4.56,-1.3 0.65,0.16 0.38,1.27 0.55,-0.18 -0.26,0.83 -0.21,-0.47 -1.25,0.04 0.41,-1.09 -0.67,-0.43 0.4,-0.13 z m -0.81,-1.47 0.9,0.87 -1.33,-0.1 0.43,-0.77 z m -3.62,-2.17 0.56,0.93 -0.85,0.56 -0.43,-0.67 0.72,-0.82 z m 2.01,-0.4 0.94,2.34 -1.03,-0.85 0.09,-1.49 z m 0.41,-0.85 0.41,0.44 -0.26,0.62 -0.63,-0.79 0.48,-0.27 z m -14.24,-2.19 1.39,1.61 -1.55,0.1 -0.63,-1.01 0.79,-0.7 z m 2.62,-1.99 0.67,0.53 -0.74,0.07 -1.33,-1.36 -0.02,-1.67 0.47,1.79 1.05,-0.63 -0.1,1.27 z m -65.0999998,-86.2 0.88,0.19 0.6,1.53 -1.02,0.43 -0.73,-1.53 -0.45,0.03 -0.06,0.82 -0.6,-0.11 0.24,-0.9 1.14,-0.46 z m -5.35,-2.13 0.48,0.71 -1.23,0.49 -0.45,-0.32 0.02,0.38 1.1,0.84 1.93,-0.34 -0.1,0.81 1.17,1.07 -2.38,0.35 -1.67,-1.25 -0.33,-2.16 1.46,-0.58 z m 47.9399998,-37.38 5.08,1.55 0,0 2.51,1.93 1.99,2.37 0,0 1.17,1.64 0,0 6.23,8.13 0,0 1.72,0.64 1.81,-0.32 0,0 3.28,-1.38 4.39,-1.03 0,0 4.46,-0.83 0,0 5.16,0.72 4.62,1.8 8.550004,0.72 0,0 1.57,-0.57 0,0 2.58,-6.05 0,0 1.26,-1.48 0,0 1.58,-0.72 0,0 2.87,-2.88 0,0 0.77,-0.41 1.57,0.95 0,0 0.46,1.64 0,0 -0.45,7.19 0.31,4.39 0,0 0.57,2.64 0,0 0.75,1.64 0,0 3.48,3.55 2.42,1.21 0,0 3.31,1.1 0,0 10.14,1.18 0,0 2.4,-1.75 0.79,-0.04 0,0 1.54,1.26 0,0 0.78,0.47 0,0 1.64,-0.31 0,0 0.75,0.21 0.79,2.47 1.29,1.08 0,0 1.73,-0.09 3.56,-1.88 0,0 3.39,-0.99 1.64,0.49 1.4,1.22 0,0 1.13,1.32 2.36,0.52 0,0 4.08,0.72 0,0 1.54,0.24 0,0 4.95,1.31 2.94,2.27 0,0 1.3,1.6 0,0 2.31,0.67 0,0 0.75,0.31 1.33,2.42 0.78,0.46 0,0 2.36,0.99 1.41,1.64 0,0 1.04,2.51 2.24,9.37 0,0 0.39,1.63 0,0 0.14,0.92 0,0 1.4,3.91 3.73,2.15 0,0 0.81,0.36 0,0 2.96,1.84 0,0 0.35,0.76 0,0 -0.29,2.41 -4.44,4.13 0,0 -3.13,3.74 -2.33,7.77 -1.99,2.39 0,0 -0.56,-1.01 0,0 -1.22,-1.43 0,0 -0.77,-0.43 0,0 -1.53,0.08 0,0 -10.45,2.03 0,0 -6.57,0.52 0,0 -2.26,-0.4 -2.42,-1.83 0,0 -1.52,-1.23 0,0 -2.27,-1.43 -2.36,-0.58 0,0 -2.35,-0.83 0,0 -0.96,-3.08 -1.28,-1.17 0,0 -0.75,0.04 -0.42,0.73 0.31,1.51 0,0 0.52,1.5 -0.8,0.64 -6.15,-2.51 -1.65,0.33 -1.68,1.29 0,0 -1.54,1.37 0,0 -1.57,1.19 -0.75,0.01 0,0 -0.74,-0.34 -0.19,-0.75 0.43,-2.4 0,0 -0.15,-0.75 0,0 -2.27,-1.42 0,0 -1.47,-1.04 -0.15,-0.79 0,0 0.2,-1.59 0,0 -1,-1.24 -5.38,-0.23 0,0 -1.52,-0.26 0,0 -0.81,-0.56 0,0 -1.09,-1.55 0,0 -2.45,-4.27 0,0 -0.56,-1.68 0,0 -0.8,-3.16 0,0 -1.11,-1.52 0,0 -0.77,-0.31 0,0 -1.55,0.46 0,0 -5.85,3.33 -1.73,0.42 -2.42,-0.31 0,0 -2.350004,-1.74 0,0 -3.96,-1.66 0,0 -1.55,0.39 0,0 -0.76,0.34 0,0 -1.64,1.06 0,0 -3.22,1.5 0,0 -1.59,0.57 0,0 -1.67,0.81 0,0 -1.62,0.43 0,0 -3.12,0.69 0,0 -1,1.47 0.09,0.75 0.81,0.48 0,0 3.12,-0.68 0,0 1.54,0.18 0,0 1.43,1.38 0,0 0.99,4.2 0,0 -0.81,8.93 0.69,2.63 1,1.58 0,0 1.6,1.31 0,0 4.78,1.35 0,0 1.54,0.95 0,0 0.36,1.18 0,0 -0.26,1.76 -3.15,5.54 0,0 -0.16,2.33 0,0 0.67,1.68 0,0 1.69,3.17 0.81,3.12 0,0 0.28,1.56 1.95,-1.78 0,0 1.53,-0.17 4.010004,2.52 0,0 2.23,1.16 0,0 0.73,0.62 0.54,2.27 0,0 0.67,1.64 2.17,1.85 0,0 3.72,3.33 1.8,3.88 0,0 1.79,6.32 0,0 0.74,0.31 0,0 3.17,0.13 0,0 1.12,1.4 0,0 -0.25,2.3 0,0 -0.1,0.74 0,0 -0.84,4.77 0,0 0.22,10.27 0,0 0.12,3.18 0,0 -0.22,1.56 -0.92,1.54 -1.61,0.83 -2.58,0.03 0,0 -2.31,-0.45 -1.55,0.41 0,0 -0.55,0.71 0,0 -1.17,1.81 0,0 -0.79,0.71 0,0 -2.36,0.91 0,0 -5.65,0.48 -0.95,1.47 -0.550004,3.24 0,0 -1.62,-2.9 0.14,-0.69 -0.84,0.33 -0.56,-1.64 -2.78,-1.51 0.33,-0.45 -0.77,-1.66 0.84,0.07 -0.05,-1.24 -0.92,-0.35 0.42,0.41 -0.78,0.58 -1.33,-1.78 0.74,-0.6 1.35,0.3 -0.89,0.44 1.2,0.37 0.35,-1.18 1.15,-0.94 -0.69,-1.49 1.33,-3.11 -1.45,-2.83 0.79,-1.46 -0.59,-2.62 -4.67,-5.22 -1.37,-3.49 0.09,-1.57 -2.7,-2.19 0.04,-1.78 -0.93,-0.25 -0.22,-2.07 -1.07,-1.57 -2.11,-0.91 -2.23,-3.18 -0.39,0.01 0.53,1.51 -0.37,0.18 -0.61,-0.06 -0.53,-0.89 -0.9,-0.06 -1.1,-0.86 0.18,-1.06 -0.49,0.39 -0.45,-0.52 -0.58,-1.92 -0.96,-0.3 1.62,-2.56 0.64,-2.31 -0.24,-1.94 -1.79,-1.12 -2.77,-0.16 -0.28,-0.17 -2.35,-1.96 -1.09,-3.69 -3.07,-4.99 -2.17,-5.7 -2.64,-4.47 -2.02,-2.17 -1.02,-1.98 -1.75,-0.97 -1.79,-4.21 -0.16,-1.73 -3.01,-3.32 -1.93,-1.1 -0.29,-0.81 -3.08,-3.08 -0.43,-2.1 1.01,-1.06 -0.31,-1.15 -2.3,-3.78 -2.56,-1.31 -2.59,-2.96 -0.18,-1.36 -1.79,-2.4 -0.4,-3.1 -1.48,-1.89 -0.38,-1.99 -1.02,-1.86 -0.85,-0.86 -1.85,-0.56 -0.19,-2.51 -2.49,-2 -0.65,-1.62 -3.32,-1.72 -0.19,-0.58 2.18,-0.65 -0.07,-0.59 -2,-0.36 -0.68,-0.59 -1.3,0.06 -1.7,-1.66 -0.56,0.22 -0.9,-0.6 0,0.26 0.51,0.81 -1.1,-0.65 -2.4,1.32 -2.25,-0.17 -0.53,0.41 -0.23,-0.96 1.02,-0.56 -1.66,0.11 -0.58,1.35 -0.27,-0.42 0.64,-1.12 -0.56,0.08 -0.54,0.73 -1.0199998,-0.16 0.44,1.27 -0.96,0.02 0.44,-1.68 -1.35,-0.19 0.32,-0.33 -0.44,-0.83 -0.25,0.49 -0.72,-0.3 -1.13,1.56 0.59,1.1 -0.68,0.14 -0.71,1.44 -0.78,-1.45 0.2,-1.28 -0.53,-0.22 -0.75,0.5 0.01,-0.79 1.96,-2.53 0.97,0.36 -0.42,-1.04 0.6,-0.43 0.76,-2.54 -0.34,-0.08 3.31,-8.17 1.1499998,-1.73 -0.27,-3.51 -0.7699998,-1.93 1.9199998,-5.76 0.03,-2.95 0,0 3.23,0.16 1.64,-0.61 2.49,0.78 0,0 5.25,0.69 0,0 1.92,-0.12 2.31,-2.25 0,0 3.26,-0.84 0,0 6.03,-1.09 0,0 2.35,0.22 0,0 3.31,-0.69 0,0 3.38,-0.17 0,0 2.61,0.51 z" data-region="tabuk" />
      <path id="region-northern" d="m 403.89718,130.68041 -12.46,4.39 -8.25,8.72 -12.66,23.68 -9.15,12.15 -1.29,4.67 0.39,1.52 1.47,1.83 0,0 -1.3,2.23 -1.8,0.86 -1.51,-0.52 -1.94,-2.12 0,0 -1.03,-1.87 -6.08,-5.52 -1.62,-0.95 -3.28,-1.26 -9.98,-1.29 -2.44,-1.44 -2.02,-2.48 -2.78,-7.7 -5.28,-9.66 -1.08,-1.62 -1.58,-1.23 -1.57,0.21 -5.05,3.38 -2.52,0.84 -6,-3.12 -6.49,0.31 -0.96,-0.31 -1.42,-1.46 -0.88,-6.22 -0.67,-0.8 -2.68,0.47 -4.62,2.48 -3.99,0.21 -2.52,-1.46 -4.2,-5.16 -2.37,-1.99 -14.47,-3.51 -4.45,0.36 -4.76,1.35 -12.17,-3.53 -20.96,4.87 -2.37,-0.37 -4.38,-1.9 0,0 5.62,-5.69 0.89,-6.06 0.39,-9.36 1.4,-2.47 6.16,-4.72 27.27,-16.849995 5.26,-4.73 -0.1,-1.57 -7.78,-6.26 -3.13,-6.39 -2.09,-2.5 -3.6,-1.59 -5.45,0.04 -3.23,-0.65 -7.57,-6.08 -2.7,-1.26 -7.01,0.88 -17.95,-0.62 -3.7,-1.12 -3.98,-3.44 -1.78,-0.77 -4.38,-0.08 -19.01,-3.45 -8.05,0.14 -7.25,1.36 -13.58,1.49 -3.42,-0.04 -2.31,-0.62 -2.19,-1.9 -3.89,-6.96 -4.7,-3.69 -0.58,-6.72 2.84,-8.34 1.27,-6.06 0,0 35.03,-10.31 10.52,-9.1700004 -0.45,-2.01 1.16,1.54 2.33,1.41 18.56,5.42 6.07,2.8400004 3.42,2.31 88.69,62.66 66.05,45.649995 7,1.62 z" data-region="northern" />
      <path id="region-jazan" d="m 260.53718,593.84041 0.42,0.19 0.18,-0.09 0.34,0.26 -0.65,1.12 -0.68,-0.76 0.39,-0.72 z m -0.48,-3.22 0.36,0.53 -0.42,1.5 -0.08,-0.96 -0.49,0.77 -0.56,-1.17 0.35,-0.56 0.84,-0.11 z m -6.48,-0.49 1.42,1.06 -0.8,-0.19 -0.62,-0.87 z m -3.48,-2.47 1.93,0.5 1.13,1.63 -3.2,-1.03 -0.65,-0.76 0.79,-0.34 z m 9.66,-3.75 0.49,0.34 -1.62,0.14 1.13,-0.48 z m -9.18,-1.81 3.17,1.84 1.47,3.11 2.35,0.67 1.59,-0.36 0.99,1.34 1.6,-0.47 -0.28,-0.9 -0.44,-0.07 0.28,0.62 -0.84,0.04 -0.43,-0.61 0.52,-0.58 0.1,-1.28 0.59,0.1 0.18,-0.63 0.86,0.1 0.21,1.43 0.76,1.02 0.42,-0.08 0.59,1 0.91,-0.21 0.03,0.51 0.47,-0.06 -0.08,0.94 -0.62,0.65 0.44,2.97 -0.93,0.11 -1.24,-1.42 -1.81,-0.37 1.24,-0.6 -0.09,-1.26 -1.8,-0.44 -3.04,0.93 -1.97,-1.64 -1.14,-0.32 -1.19,-1.03 -0.96,-1.9 -1.33,-0.03 0.46,-1.25 -1,-0.81 -0.04,-1.06 z m -6.47,-0.16 2.05,3.05 -1.59,-0.6 0.21,-0.33 -0.75,-0.69 0.08,-1.43 z m 11.93,-2.08 1.14,0.78 -0.56,0.94 0.58,0.92 1.38,0.42 -0.13,0.87 -1.3,-0.32 -0.74,0.59 2.19,1.66 0.74,1.04 -0.28,0.45 -0.76,-1.15 -3.02,-1 -1.83,-2.61 0.01,-1.08 1.64,-0.2 0.94,-1.31 z m -8.03,-0.22 1.51,1.1 -0.69,1.16 -1,0.31 -0.94,-1.16 1.12,-1.41 z m 3.62,-0.86 1.13,0.65 -0.47,0.57 -0.44,-0.06 -0.22,-1.16 z m 5.16,0.24 0.42,1.41 -2.06,-2.08 1.64,0.67 z m -5.78,-47.93 4.46,-0.47 1.26,1.03 -0.82,7.27 0.28,1.5 0.74,0.47 2.95,-0.45 2.15,0.23 1.08,1.35 0.85,2.89 0.73,0.64 8.28,0.4 1.28,1.24 1.59,3.73 3.25,3.17 1.52,0.63 1.41,-0.14 0.56,-0.66 1.67,-6.75 1.21,-2.1 2.87,-2.16 2.21,-0.88 1,1.12 -0.13,4.42 3.6,1.82 2.64,4.64 0.69,0.57 2.95,0.6 1.98,3.01 0,0 0.97,1.39 -1.92,2.92 0.09,2.37 1.55,0.9 0,0.65 0.81,0.35 0.26,0.69 -4.49,1.41 0.42,1.87 -1.52,1.37 0.05,1.45 -0.62,0.32 -0.06,0.88 0.09,0.35 0.98,-0.4 1.63,0.48 0.82,1.64 -0.55,0.91 -2.27,-0.26 0.26,2.81 -1.5,0.7 -0.04,0.6 1.45,1.89 1.55,0.34 0.33,0.83 1.01,0.48 -0.22,1.26 0.48,0.86 -1.43,0.11 -0.32,0.81 0.49,2.6 -0.84,0.15 -2.07,-1.11 -0.66,0.18 0.44,0.9 -1.03,2.74 0.05,1.06 -4.42,1.16 -1.67,1.19 0.42,2.71 -1.71,0.38 -4.82,-0.49 -0.34,-0.69 0.6,-1.77 -1.21,-1.16 -0.84,-2.21 0.56,-3.56 -0.33,-0.29 -0.47,-0.6 0,-0.93 0.01,-0.57 -2.51,-3.11 -1.41,-0.69 -0.64,0.25 -0.39,-1.35 -0.51,-0.19 -0.69,-0.31 1.07,-0.96 -0.53,-1.39 0.31,-1.85 -1.37,-1.12 -2.17,-0.67 -1.33,-1.88 0.23,-1.83 -0.46,-0.18 -1.12,-0.67 -0.26,0.3 1.13,1.95 -0.66,0.66 0.05,1.98 -0.3,0.95 -0.18,-3.73 -0.98,-3.19 -0.53,-4.16 -0.56,-0.31 0.66,-2.11 -0.29,-1.63 -1.44,-0.38 -1.57,-2.47 -1.97,-1.51 -0.98,0.05 -0.2,-1.49 -0.79,-0.94 -7.32,-5.21 -0.65,-1.03 -1.45,-0.33 -1.24,-1.02 -1.05,-0.03 0.19,-0.8 -3.78,-3.48 0.69,-0.01 -1.01,-0.91 0.26,-1.07 -0.59,0.79 -0.49,-0.33 -0.12,-0.55 0.59,-0.18 -0.22,-1.01 -1.39,-0.54 -0.76,-1.79 0,0 1.97,-3.15 3.55,-2.05 0.98,-2.11 z" data-region="jazan" />
      <path id="region-najran" d="m 399.93718,569.40041 -17.23,-2.36 -6.25,-0.42 -6.25,-4.03 -19.73,0 -1.39,0.98 -1.95,0.27 -2.64,-1.39 -2.7,0.12 -8.79,1.12 -0.82,1.52 -1.08,-1.34 -2.95,0.53 -1.37,1.92 -2.61,-0.72 -1.6,0.43 -1.97,-0.29 -0.68,-0.89 -2.02,0.36 -2.59,-2.7 0.07,-1.04 0,0 1.15,-3.24 -1.3,-5.93 0.67,-1.47 0.77,-8.37 -1.38,-6.12 0.08,-1.66 0.88,-1.69 3.08,-3.43 8.21,-7.56 1.08,-1.48 1.06,-3.02 0.95,-1.05 6.32,-2.02 2.38,-1.42 1.22,-1.41 0.97,-2.22 0.17,-2.12 -1.76,-8.57 0.69,-1.59 3.85,-1.94 1.12,-1.39 0.04,-6.08 0,0 8.51,5.59 5.22,2.39 3.24,0.72 8.85,0.7 58.85,-5.04 16.39,-2.39 10.13,-2.63 0,0 -7.99,63.49 -2.59,24.45 0,0 -7.48,4.72 -7.03,-0.36 -8.18,-11.04 -10.7,1.53 z" data-region="najran" />
      <path id="region-bahah" d="m 260.01718,447.12041 -2.15,6.42 0.23,0.79 2.05,1.97 0.11,1.59 -1.04,1.45 -5.82,3.97 -0.84,1.79 0.23,3.55 -4.26,8.69 0,0 -0.79,0.51 -4.62,0.04 -1.52,0.58 -2.4,4.46 -2.58,8.99 -2.21,1.08 -2.58,0 -1.59,-1.05 -2.06,-3.06 -3.68,-2.13 -1.3,-1.54 0.26,-4.43 -0.91,-3.89 0.31,-5.29 -0.97,-1.52 -3.65,-1.1 -0.67,-0.7 -0.18,-1.41 4.01,-4.1 7.73,-1.86 1.37,-3.56 2.69,-4.63 0.12,-2.35 -1.11,-4.55 0.7,-2.11 1.36,-1.42 2.26,-0.31 1.43,0.61 1.21,1.45 1.28,3.78 1.92,1.46 4.61,-1.77 4.17,0.2 5.41,-2.75 1.49,0.22 z" data-region="bahah" />
      <path id="region-jawf" d="m 13.437176,126.05041 0.66,-2.82 0.74,-0.76 -0.18,-0.82 1.04,-0.61 -0.48,-1.92 0.81,-0.46 -0.24,-0.86 0.61,-1.36 38.21,6.79 14.65,-12.07 8.85,-14.779995 25.760004,-5.16 5.78,-13.18 11.32,-6.78 -34.260004,-39.72 33.690004,-9.9 0,0 -1.27,6.06 -2.84,8.34 0.58,6.72 4.7,3.69 3.89,6.96 2.19,1.9 2.31,0.62 3.42,0.04 13.58,-1.49 7.25,-1.36 8.05,-0.14 19.01,3.45 4.38,0.08 1.78,0.77 3.98,3.44 3.7,1.12 17.95,0.62 7.01,-0.88 2.7,1.26 7.57,6.08 3.23,0.65 5.45,-0.04 3.6,1.59 2.09,2.5 3.13,6.39 7.78,6.26 0.1,1.57 -5.26,4.73 -27.27,16.849995 -6.16,4.72 -1.4,2.47 -0.39,9.36 -0.89,6.06 -5.62,5.69 0,0 -3.04,-0.66 -12.9,5.53 -13.07,7.6 -4.29,0.69 -9.93,4.38 -6.27,3.54 -3.69,4.2 0,0 -1.41,-1.21 -1.63,-0.49 -3.39,1 0,0 -3.56,1.88 -1.73,0.08 0,0 -1.3,-1.08 -0.78,-2.47 -0.75,-0.21 0,0 -1.64,0.32 0,0 -0.77,-0.47 0,0 -1.54,-1.26 0,0 -0.8,0.04 -2.4,1.75 0,0 -10.13,-1.18 0,0 -3.32,-1.1 0,0 -2.42,-1.21 -3.47,-3.55 0,0 -0.75,-1.63 0,0 -0.57,-2.65 0,0 -0.31,-4.39 0.45,-7.19 0,0 -0.46,-1.64 0,0 -1.57,-0.94 -0.77,0.41 0,0 -2.87,2.88 0,0 -1.58,0.72 0,0 -1.26,1.47 0,0 -2.58,6.06 0,0 -1.57,0.57 0,0 -8.550004,-0.71 -4.62,-1.8 -5.16,-0.72 0,0 -4.47,0.84 0,0 -4.39,1.03 -3.28,1.37 0,0 -1.81,0.33 -1.72,-0.64 0,0 -6.23,-8.13 0,0 -1.17,-1.64 0,0 -1.99,-2.37 -2.51,-1.93 0,0 -5.08,-1.54 0,0 -2.61,-0.51 0,0 -3.38,0.17 0,0 -3.31,0.69 0,0 -2.35,-0.22 0,0 -6.03,1.09 0,0 -3.27,0.83 0,0 -2.3,2.26 -1.93,0.12 0,0 -5.24,-0.69 0,0 -2.49,-0.78 -1.64,0.61 -3.23,-0.16 0,0 0.39,-2.96 0.64,-1.27 -0.41,-1.66 0.82,-1.35 z" data-region="jawf" />
      <path id="region-asir" d="m 303.24718,558.19041 -1.98,-3.01 -2.95,-0.6 -0.68,-0.57 -2.65,-4.63 -3.6,-1.82 0.14,-4.42 -1,-1.12 -2.21,0.88 -2.87,2.16 -1.21,2.1 -1.66,6.75 -0.57,0.66 -1.4,0.14 -1.52,-0.63 -3.25,-3.16 -1.59,-3.73 -1.28,-1.24 -8.28,-0.4 -0.73,-0.64 -0.85,-2.89 -1.08,-1.35 -2.16,-0.23 -2.94,0.45 -0.74,-0.47 -0.28,-1.5 0.83,-7.27 -1.27,-1.03 -4.46,0.47 0,0 1.5,-6.41 -2.97,-9.14 -2.11,-1.2 -6.71,-0.21 -1.43,-0.49 -1.03,-1.47 -0.61,-2.97 0.49,-10 1.97,1.88 0.75,-0.06 3.87,-2.82 2.24,-0.38 4.74,0.44 1.52,-0.75 0.76,-1.42 1.27,-4.6 -2.19,-4.44 1.31,-6.92 -0.21,-1.51 -1.85,-1.89 -2.29,-0.13 -1.5,0.74 0,0 4.26,-8.69 -0.23,-3.55 0.84,-1.79 5.82,-3.97 1.04,-1.45 -0.11,-1.59 -2.05,-1.97 -0.23,-0.79 2.15,-6.42 0,0 4.67,9.08 1.4,1.02 2.28,0.24 2.31,-1.13 10.11,-9.25 6.41,-3.06 4.26,-3.8 4.55,-0.28 3.16,-1.52 2.26,-0.24 5.21,1.3 5.72,-3.77 4.02,-1.46 0,0 5.76,5.01 3.07,4.93 3.6,3.6 1.06,1.73 -0.08,2.96 -3.34,6.09 -1.23,6.11 0.22,1.6 2.75,2.96 3.91,6.36 13.54,12.09 0,0 -0.04,6.08 -1.12,1.39 -3.85,1.94 -0.69,1.59 1.76,8.57 -0.17,2.12 -0.97,2.22 -1.22,1.41 -2.38,1.42 -6.32,2.02 -0.95,1.05 -1.06,3.02 -1.08,1.48 -8.21,7.56 -3.08,3.43 -0.88,1.69 -0.08,1.66 1.38,6.12 -0.77,8.37 -0.67,1.47 1.3,5.93 -1.15,3.24 0,0 -0.11,1.8 -2.37,-2.3 -5.29,-3.37 -2.46,-0.34 z" data-region="asir" />

      <!-- Text Labels -->
      <text x="347.1771799999998" y="322.36541000000005" text-anchor="middle" fill="rgba(255,255,255,0.9)" font-size="12" font-weight="700" pointer-events="none" style="text-shadow: 0px 1px 3px rgba(0,0,0,0.8);">الرياض</text>
      <text x="200.04717999999994" y="440.85541000000035" text-anchor="middle" fill="rgba(255,255,255,0.9)" font-size="12" font-weight="700" pointer-events="none" style="text-shadow: 0px 1px 3px rgba(0,0,0,0.8);">مكة المكرمة</text>
      <text x="170.51717800000006" y="285.6404100000001" text-anchor="middle" fill="rgba(255,255,255,0.9)" font-size="12" font-weight="700" pointer-events="none" style="text-shadow: 0px 1px 3px rgba(0,0,0,0.8);">المدينة المنورة</text>
      <text x="574.99718" y="310.7554099999998" text-anchor="middle" fill="rgba(255,255,255,0.9)" font-size="12" font-weight="700" pointer-events="none" style="text-shadow: 0px 1px 3px rgba(0,0,0,0.8);">الشرقية</text>
      <text x="297.73718" y="247.28041000000002" text-anchor="middle" fill="rgba(255,255,255,0.9)" font-size="12" font-weight="700" pointer-events="none" style="text-shadow: 0px 1px 3px rgba(0,0,0,0.8);">القصيم</text>
      <text x="255.4671799999999" y="203.88540999999998" text-anchor="middle" fill="rgba(255,255,255,0.9)" font-size="12" font-weight="700" pointer-events="none" style="text-shadow: 0px 1px 3px rgba(0,0,0,0.8);">حائل</text>
      <text x="98.05717809999997" y="236.55540999999982" text-anchor="middle" fill="rgba(255,255,255,0.9)" font-size="12" font-weight="700" pointer-events="none" style="text-shadow: 0px 1px 3px rgba(0,0,0,0.8);">تبوك</text>
      <text x="260.0921800000001" y="95.44041230000002" text-anchor="middle" fill="rgba(255,255,255,0.9)" font-size="12" font-weight="700" pointer-events="none" style="text-shadow: 0px 1px 3px rgba(0,0,0,0.8);">الحدود الشمالية</text>
      <text x="269.52717999999993" y="570.3854100000001" text-anchor="middle" fill="rgba(255,255,255,0.9)" font-size="12" font-weight="700" pointer-events="none" style="text-shadow: 0px 1px 3px rgba(0,0,0,0.8);">جازان</text>
      <text x="386.03218000000004" y="533.3604100000001" text-anchor="middle" fill="rgba(255,255,255,0.9)" font-size="12" font-weight="700" pointer-events="none" style="text-shadow: 0px 1px 3px rgba(0,0,0,0.8);">نجران</text>
      <text x="228.82218" y="462.49541" text-anchor="middle" fill="rgba(255,255,255,0.9)" font-size="12" font-weight="700" pointer-events="none" style="text-shadow: 0px 1px 3px rgba(0,0,0,0.8);">الباحة</text>
      <text x="133.8071779999999" y="92.95041249999998" text-anchor="middle" fill="rgba(255,255,255,0.9)" font-size="12" font-weight="700" pointer-events="none" style="text-shadow: 0px 1px 3px rgba(0,0,0,0.8);">الجوف</text>
      <text x="291.6421800000001" y="498.76040999999975" text-anchor="middle" fill="rgba(255,255,255,0.9)" font-size="12" font-weight="700" pointer-events="none" style="text-shadow: 0px 1px 3px rgba(0,0,0,0.8);">عسير</text>
    </svg>`;

        const tooltip = document.getElementById('map-tooltip');
        const paths = container.querySelectorAll('path[data-region]');

        // Apply mode CSS class
        if (activeMapMode === 'cities') {
            container.classList.add('cities-mode-active');
        } else {
            container.classList.remove('cities-mode-active');
        }

        paths.forEach(path => {
            const adminId = path.dataset.region;
            const geoGroupId = REGION_MAPPING[adminId];

            // Color based on geo group
            path.classList.add(`group-${geoGroupId}`);

            if (activeMapMode === 'regions') {
                // Regions mode: highlight completed geo groups
                if (completedRegions.includes(geoGroupId)) {
                    path.classList.add('group-completed');
                }
                // Click → show region clothing
                path.addEventListener('click', e => {
                    e.stopPropagation();
                    selectRegionGroup(geoGroupId);
                    showRegionInfo(geoGroupId);
                });
            } else {
                // Cities mode: highlight completed admin cities
                if (completedCities.includes(adminId)) {
                    path.classList.add('group-completed');
                }
                // Click → show city facts
                path.addEventListener('click', e => {
                    e.stopPropagation();
                    selectRegionGroup(adminId);
                    showCityInfo(adminId);
                });
            }

            // Hover highlighting
            path.addEventListener('mouseenter', () => highlightGroup(geoGroupId));
            path.addEventListener('mouseleave', () => clearHighlight());

            // Tooltip
            path.addEventListener('mousemove', e => {
                if (!tooltip) return;
                const data = HawdajData.get();
                let name;
                if (activeMapMode === 'cities') {
                    name = HawdajData.CITIES_DATA[adminId]?.name || adminId;
                } else {
                    name = data.regions[geoGroupId]?.name || geoGroupId;
                }
                tooltip.textContent = name;
                tooltip.style.left = e.clientX + 'px';
                tooltip.style.top = (e.clientY - 15) + 'px';
                tooltip.style.display = 'block';
            });
        });

        // Labels: in regions mode → 5 big group labels; in cities mode → individual province labels
        const texts = container.querySelectorAll('text');
        const svg = container.querySelector('svg');

        if (activeMapMode === 'regions') {
            // Hide all individual city labels
            texts.forEach(t => { t.style.display = 'none'; });

            // 5 geographic group labels with emojis (kid-friendly)
            const GROUP_LABELS = [
                { id: 'central',  x: 370, y: 335, emoji: '🏰', name: 'المنطقة الوسطى',   textColor: '#7A3500' },
                { id: 'eastern',  x: 590, y: 390, emoji: '🛢️',  name: 'المنطقة الشرقية',  textColor: '#003D5C' },
                { id: 'northern', x: 225, y: 178, emoji: '🏔️', name: 'المنطقة الشمالية', textColor: '#3A005C' },
                { id: 'western',  x: 165, y: 398, emoji: '🕌', name: 'المنطقة الغربية',  textColor: '#5C0025' },
                { id: 'southern', x: 298, y: 535, emoji: '🌿', name: 'المنطقة الجنوبية', textColor: '#0A4020' },
            ];

            GROUP_LABELS.forEach(gl => {
                const done = completedRegions.includes(gl.id);
                // Background rect
                const rect = document.createElementNS('http://www.w3.org/2000/svg', 'rect');
                const labelText = `${gl.emoji} ${gl.name}${done ? ' ✅' : ''}`;
                rect.setAttribute('x', gl.x - 68);
                rect.setAttribute('y', gl.y - 20);
                rect.setAttribute('width', 136);
                rect.setAttribute('height', 26);
                rect.setAttribute('rx', 13);
                rect.setAttribute('fill', 'rgba(0,0,0,0.45)');
                rect.setAttribute('pointer-events', 'none');
                svg.appendChild(rect);

                const txt = document.createElementNS('http://www.w3.org/2000/svg', 'text');
                txt.setAttribute('x', gl.x);
                txt.setAttribute('y', gl.y - 2);
                txt.setAttribute('text-anchor', 'middle');
                txt.setAttribute('dominant-baseline', 'middle');
                txt.setAttribute('fill', '#FFFFFF');
                txt.setAttribute('font-size', '13');
                txt.setAttribute('font-weight', '900');
                txt.setAttribute('pointer-events', 'none');
                txt.style.cssText = 'font-family: Changa, Tajawal, sans-serif; text-shadow: 0 1px 4px rgba(0,0,0,0.9);';
                txt.textContent = labelText;
                svg.appendChild(txt);
            });
        } else {
            // Cities mode: individual province names already in SVG — just show them
            texts.forEach(t => { t.style.display = ''; });
        }

        container.addEventListener('mouseleave', () => {
            if (tooltip) tooltip.style.display = 'none';
        });
    }

    // ── Cities Mode ───────────────────────────────────────────────────────
    function showCityInfo(cityId) {
        lastClickedId = cityId;
        const cityData = HawdajData.CITIES_DATA[cityId];
        if (!cityData) return;

        stopSpeech();

        // Set header
        document.getElementById('region-name').textContent = cityData.name;
        document.getElementById('region-capital').textContent = `العاصمة: ${cityData.capital}`;

        // Show "استمع" button
        const readBtn = document.getElementById('btn-read-facts');
        if (readBtn) {
            readBtn.style.display = 'flex';
            readBtn.classList.remove('speaking');
            readBtn.querySelector('.btn-text').textContent = 'استمع للمعلومات';
            readBtn.onclick = () => handleReadFacts(cityId, cityData.facts, readBtn, 'cities');
        }

        // Build visual fact cards
        const factsContainer = document.getElementById('region-facts');
        factsContainer.innerHTML = '';
        const rawFacts = cityData.facts || '';
        const factLines = rawFacts.split('\n').map(l => l.replace(/^•\s*/, '').trim()).filter(Boolean);
        const grid = document.createElement('div');
        grid.className = 'fact-cards-grid';
        factLines.forEach((line, i) => {
            const card = document.createElement('div');
            card.className = 'fact-card';
            card.innerHTML = `<span class="fact-card-emoji">${FACT_EMOJIS[i % FACT_EMOJIS.length]}</span><span class="fact-card-text">${line}</span>`;
            grid.appendChild(card);
        });
        factsContainer.appendChild(grid);

        // Hide clothing container in cities mode
        const clothingContainer = document.getElementById('region-clothing-container');
        if (clothingContainer) clothingContainer.style.display = 'none';

        // Render questions
        renderQuestions(cityData.questions || [], 'city');

        document.getElementById('region-modal').classList.add('active');
        HawdajAudio.SFX.tap();
    }

    function handleReadFacts(id, factsText, btn, mode = 'cities') {
        if (isSpeaking) {
            stopSpeech();
            btn.classList.remove('speaking');
            btn.querySelector('.btn-text').textContent = 'استمع للمعلومات';
            return;
        }

        // Try local MP3 first
        const audioPath = `assets/audio/map/${mode}/${id}_facts.mp3`;
        const audio = new Audio(audioPath);
        audio.onerror = () => {
            // Fallback to browser TTS
            useTTS(factsText, btn);
        };
        audio.onplay = () => {
            isSpeaking = true;
            btn.classList.add('speaking');
            btn.querySelector('.btn-text').textContent = '⏹ إيقاف';
        };
        audio.onended = audio.onpause = () => {
            isSpeaking = false;
            btn.classList.remove('speaking');
            btn.querySelector('.btn-text').textContent = 'استمع للمعلومات';
        };
        currentAudio = audio;
        audio.play().catch(() => useTTS(factsText, btn));
    }

    function useTTS(text, btn) {
        if (!window.speechSynthesis) return;
        stopSpeech();
        const cleanText = text.replace(/•/g, '').replace(/\n/g, '، ');
        const utter = new SpeechSynthesisUtterance(cleanText);
        utter.lang = 'ar-SA';
        utter.rate = 0.9;
        utter.onstart = () => {
            isSpeaking = true;
            if (btn) { btn.classList.add('speaking'); btn.querySelector('.btn-text').textContent = '⏹ إيقاف'; }
        };
        utter.onend = utter.onerror = () => {
            isSpeaking = false;
            if (btn) { btn.classList.remove('speaking'); btn.querySelector('.btn-text').textContent = 'استمع للمعلومات'; }
        };
        window.speechSynthesis.speak(utter);
    }

    function stopSpeech() {
        if (currentAudio) { currentAudio.pause(); currentAudio = null; }
        if (window.speechSynthesis) window.speechSynthesis.cancel();
        isSpeaking = false;
    }

    // ── Regions Mode ──────────────────────────────────────────────────────
    function showRegionInfo(regionId) {
        lastClickedId = regionId;
        const data = HawdajData.get();
        const region = data.regions[regionId];
        if (!region) return;

        stopSpeech();

        document.getElementById('region-name').textContent = region.name;
        document.getElementById('region-capital').textContent = `المدن الرئيسية: ${region.capital}`;

        // Show listen button in regions mode
        const readBtn = document.getElementById('btn-read-facts');
        if (readBtn) {
            readBtn.style.display = 'flex';
            readBtn.classList.remove('speaking');
            readBtn.querySelector('.btn-text').textContent = 'استمع للمعلومات';
            readBtn.onclick = () => handleReadFacts(regionId, region.facts, readBtn, 'regions');
        }

        // Facts as plain text
        const factsContainer = document.getElementById('region-facts');
        factsContainer.innerHTML = '';
        const rawFacts = region.facts || '';
        const factLines = rawFacts.split('\n').map(l => l.replace(/^•\s*/, '').trim()).filter(Boolean);
        const grid = document.createElement('div');
        grid.className = 'fact-cards-grid';
        factLines.forEach((line, i) => {
            const card = document.createElement('div');
            card.className = 'fact-card';
            card.innerHTML = `<span class="fact-card-emoji">${FACT_EMOJIS[i % FACT_EMOJIS.length]}</span><span class="fact-card-text">${line}</span>`;
            grid.appendChild(card);
        });
        factsContainer.appendChild(grid);

        // Build clothing carousel
        const clothingContainer = document.getElementById('region-clothing-container');
        if (region.fashion && Object.keys(region.fashion).length > 0) {
            clothingContainer.style.display = 'block';

            // Group by category: collect items by base key
            const categoryGroups = {};
            Object.entries(region.fashion).forEach(([key, val]) => {
                const baseKey = key.replace(/\d+$/, ''); // man2 → man, women2 → women, girl1 → girl, girl2 → girl
                const canonical = (['girl1','girl2'].includes(key)) ? 'girl' :
                                  (['women','women2'].includes(key)) ? 'woman' :
                                  (['man','man2','man3'].includes(key)) ? 'man' : baseKey;
                if (!categoryGroups[canonical]) categoryGroups[canonical] = [];
                categoryGroups[canonical].push({ key, val });
            });

            // Build ordered carousel items: boy, girl, man, woman order
            carouselItems = [];
            const categoryOrder = ['boy', 'girl', 'man', 'woman'];
            categoryOrder.forEach(cat => {
                if (categoryGroups[cat]) {
                    categoryGroups[cat].forEach(item => carouselItems.push(item));
                }
            });
            // Add any remaining categories not in order
            Object.keys(categoryGroups).forEach(cat => {
                if (!categoryOrder.includes(cat)) {
                    categoryGroups[cat].forEach(item => carouselItems.push(item));
                }
            });

            carouselIndex = 0;
            renderCarousel(clothingContainer);
        } else {
            clothingContainer.style.display = 'none';
        }

        // Questions
        renderQuestions(region.questions || [], 'region');

        document.getElementById('region-modal').classList.add('active');
        HawdajAudio.SFX.tap();
    }

    function renderCarousel(container) {
        if (!container || carouselItems.length === 0) return;

        const tabsContainer = document.getElementById('region-clothing-tabs');
        const imgEl = document.getElementById('region-clothing-display-img');
        const textEl = document.getElementById('region-clothing-display-text');

        // Build carousel UI inside clothing tabs area
        tabsContainer.innerHTML = '';

        // Category label pills (click to jump)
        const pillsRow = document.createElement('div');
        pillsRow.style.cssText = 'display:flex; justify-content:center; gap:8px; flex-wrap:wrap; margin-bottom:8px;';
        carouselItems.forEach((item, idx) => {
            const pill = document.createElement('button');
            pill.className = 'clothing-tab-btn' + (idx === carouselIndex ? ' active' : '');
            pill.textContent = FASHION_LABELS[item.key] || item.key;
            pill.setAttribute('data-idx', idx);
            pill.addEventListener('click', () => {
                carouselIndex = idx;
                updateCarouselDisplay(imgEl, textEl, pillsRow);
                HawdajAudio.SFX.tap();
            });
            pillsRow.appendChild(pill);
        });
        tabsContainer.appendChild(pillsRow);

        // Showcase box with arrows
        const showcase = container.querySelector('.clothing-showcase-box');
        if (showcase) {
            // Remove old arrows if any
            showcase.querySelectorAll('.slider-arrow-btn').forEach(b => b.remove());
            showcase.querySelector('.slider-dots-container')?.remove();

            if (carouselItems.length > 1) {
                // Prev (right in RTL)
                const prevBtn = document.createElement('button');
                prevBtn.className = 'slider-arrow-btn prev-btn';
                prevBtn.innerHTML = '›';
                prevBtn.addEventListener('click', e => { e.stopPropagation(); carouselIndex = (carouselIndex + 1) % carouselItems.length; updateCarouselDisplay(imgEl, textEl, pillsRow); HawdajAudio.SFX.tap(); });
                showcase.appendChild(prevBtn);

                // Next (left in RTL)
                const nextBtn = document.createElement('button');
                nextBtn.className = 'slider-arrow-btn next-btn';
                nextBtn.innerHTML = '‹';
                nextBtn.addEventListener('click', e => { e.stopPropagation(); carouselIndex = (carouselIndex - 1 + carouselItems.length) % carouselItems.length; updateCarouselDisplay(imgEl, textEl, pillsRow); HawdajAudio.SFX.tap(); });
                showcase.appendChild(nextBtn);

                // Dots
                const dotsRow = document.createElement('div');
                dotsRow.className = 'slider-dots-container';
                carouselItems.forEach((_, idx) => {
                    const dot = document.createElement('span');
                    dot.className = 'slider-dot' + (idx === carouselIndex ? ' active' : '');
                    dot.setAttribute('data-dot', idx);
                    dot.addEventListener('click', () => { carouselIndex = idx; updateCarouselDisplay(imgEl, textEl, pillsRow); });
                    dotsRow.appendChild(dot);
                });
                showcase.appendChild(dotsRow);
            }
        }

        updateCarouselDisplay(imgEl, textEl, pillsRow);
    }

    function updateCarouselDisplay(imgEl, textEl, pillsRow) {
        const item = carouselItems[carouselIndex];
        if (!item) return;

        if (imgEl) {
            imgEl.src = item.val.img;
            imgEl.style.display = 'block';
            imgEl.style.opacity = '0';
            imgEl.style.transition = 'opacity 0.3s ease';
            setTimeout(() => { imgEl.style.opacity = '1'; }, 30);
        }
        if (textEl) textEl.textContent = item.val.desc;

        // Update pills
        if (pillsRow) {
            pillsRow.querySelectorAll('.clothing-tab-btn').forEach(pill => {
                pill.classList.toggle('active', parseInt(pill.getAttribute('data-idx')) === carouselIndex);
            });
        }

        // Update dots
        const showcase = document.querySelector('.clothing-showcase-box');
        if (showcase) {
            showcase.querySelectorAll('.slider-dot').forEach(dot => {
                dot.classList.toggle('active', parseInt(dot.getAttribute('data-dot')) === carouselIndex);
            });
        }

        // Indicator badge
        const existing = document.querySelector('.slider-indicator-badge');
        if (existing) existing.remove();
        if (carouselItems.length > 1 && textEl) {
            const badge = document.createElement('span');
            badge.className = 'slider-indicator-badge';
            badge.textContent = `${carouselIndex + 1} / ${carouselItems.length}`;
            textEl.parentNode.insertBefore(badge, textEl.nextSibling);
        }
    }

    // ── Questions Renderer (shared) ────────────────────────────────────────
    function renderQuestions(questions, mode) {
        const qContainer = document.getElementById('region-questions');
        qContainer.innerHTML = '';

        questions.forEach((q, idx) => {
            const qDiv = document.createElement('div');
            qDiv.style.cssText = 'margin-bottom:16px; text-align:center;';

            const qText = document.createElement('p');
            qText.textContent = `${idx + 1}. ${q.question}`;
            qText.style.cssText = 'font-weight:700; margin-bottom:8px; color:var(--text-primary);';
            qDiv.appendChild(qText);

            const optGrid = document.createElement('div');
            optGrid.style.cssText = 'display:grid; grid-template-columns:1fr 1fr; gap:6px;';

            q.options.forEach((opt, optIdx) => {
                const btn = document.createElement('button');
                btn.className = 'quiz-option';
                btn.style.cssText = 'min-height:40px; font-size:var(--fs-sm); animation:none;';
                btn.textContent = opt;
                btn.addEventListener('click', () => {
                    if (btn.classList.contains('disabled')) return;
                    optGrid.querySelectorAll('.quiz-option').forEach(b => b.classList.add('disabled'));

                    if (optIdx === q.correct) {
                        btn.classList.add('correct');
                        HawdajAudio.SFX.correct();
                        HawdajEffects.sparkle(btn.getBoundingClientRect().left + btn.offsetWidth / 2, btn.getBoundingClientRect().top);
                        if (window.HawdajCelebration) HawdajCelebration.show('map');
                    } else {
                        if (window.PowerupInventory && PowerupInventory.hasShield()) {
                            PowerupInventory.consumeShield();
                            btn.style.transition = 'all 0.5s ease';
                            btn.style.transform = 'scale(0.8)';
                            btn.style.opacity = '0.3';
                            optGrid.querySelectorAll('.quiz-option').forEach(b => { if (b !== btn) b.classList.remove('disabled'); });
                            HawdajApp.showToast('🛡️ درع الحماية أعطاك فرصة أخرى!', 'success');
                            return;
                        }
                        btn.classList.add('wrong');
                        HawdajAudio.SFX.wrong();
                        optGrid.querySelectorAll('.quiz-option')[q.correct].classList.add('highlight-correct');
                        if (window.HawdajCelebration) HawdajCelebration.show('wrong');
                    }
                });
                optGrid.appendChild(btn);
            });

            qDiv.appendChild(optGrid);
            qContainer.appendChild(qDiv);
        });
    }

    // ── Close Modal ───────────────────────────────────────────────────────
    function closeRegionModal(id) {
        stopSpeech();
        document.getElementById('region-modal').classList.remove('active');
        deselectRegion();

        if (!id) return;

        if (activeMapMode === 'regions') {
            // Mark geo-region completed
            if (!completedRegions.includes(id)) {
                const data = HawdajData.get();
                completedRegions.push(id);
                data.player.completedRegions = completedRegions;
                let pts = data.points.region;
                if (window.PowerupInventory) pts = Math.floor(pts * PowerupInventory.getScoreMultiplier());
                HawdajData.addScore(pts, 'map');
                if (window.HawdajApp && HawdajApp.updatePlayerDisplay) HawdajApp.updatePlayerDisplay();
                HawdajData.save();
                if (window.completeDailyTask) window.completeDailyTask('map_explore');
                renderMap();
                updateProgress();
                HawdajApp.showToast(`🎉 اكتشفت ${HawdajData.get().regions[id]?.name}! +${HawdajData.get().points.region} نقطة`, 'success');
            }
        } else {
            // Mark city completed
            if (!completedCities.includes(id)) {
                const data = HawdajData.get();
                completedCities.push(id);
                if (!data.player.completedCities) data.player.completedCities = [];
                data.player.completedCities = completedCities;
                let pts = Math.floor((data.points.region || 15) * 0.6); // cities give 60% pts
                if (window.PowerupInventory) pts = Math.floor(pts * PowerupInventory.getScoreMultiplier());
                HawdajData.addScore(pts, 'map');
                if (window.HawdajApp && HawdajApp.updatePlayerDisplay) HawdajApp.updatePlayerDisplay();
                HawdajData.save();
                if (window.completeDailyTask) window.completeDailyTask('map_explore');
                renderMap();
                updateProgress();
                HawdajApp.showToast(`🎉 تعلمت عن ${HawdajData.CITIES_DATA[id]?.name}! +${pts} نقطة`, 'success');
            }
        }
    }

    // ── Progress ──────────────────────────────────────────────────────────
    function updateProgress() {
        const text = document.getElementById('map-progress-text');
        const fill = document.getElementById('map-progress-fill');
        if (activeMapMode === 'regions') {
            const total = 5;
            const done = completedRegions.length;
            if (text) text.textContent = `${done}/${total} مناطق`;
            if (fill) fill.style.width = `${(done / total) * 100}%`;
            if (done === total) {
                setTimeout(() => {
                    HawdajEffects.celebrationBurst();
                    HawdajAudio.SFX.victory();
                    HawdajApp.showToast('🏆 اكتشفت جميع مناطق السعودية!', 'success');
                }, 500);
            }
        } else {
            const total = 13;
            const done = completedCities.length;
            if (text) text.textContent = `${done}/${total} محافظة`;
            if (fill) fill.style.width = `${(done / total) * 100}%`;
            if (done === total) {
                setTimeout(() => {
                    HawdajEffects.celebrationBurst();
                    HawdajAudio.SFX.victory();
                    HawdajApp.showToast('🏆 استكشفت جميع محافظات السعودية!', 'success');
                }, 500);
            }
        }
    }

    // ── Helpers ───────────────────────────────────────────────────────────
    function selectRegionGroup(id) {
        selectedRegionGroup = id;
        if (activeMapMode === 'regions') highlightGroup(id);
    }

    function deselectRegion() {
        selectedRegionGroup = null;
        clearHighlight();
    }

    function highlightGroup(geoGroupId) {
        const container = document.getElementById('map-container');
        if (!container) return;
        container.querySelectorAll('path[data-region]').forEach(p => {
            const group = REGION_MAPPING[p.dataset.region];
            if (group === geoGroupId || group === selectedRegionGroup) {
                p.classList.add('highlight-group');
            } else {
                p.classList.remove('highlight-group');
            }
        });
    }

    function clearHighlight() {
        const container = document.getElementById('map-container');
        if (!container) return;
        container.querySelectorAll('path[data-region]').forEach(p => {
            const group = REGION_MAPPING[p.dataset.region];
            if (group === selectedRegionGroup) {
                p.classList.add('highlight-group');
            } else {
                p.classList.remove('highlight-group');
            }
        });
    }

    // ── Hint ──────────────────────────────────────────────────────────────
    function useHint() {
        const compassBtn = document.getElementById('btn-map-compass');
        if (compassBtn) {
            compassBtn.classList.add('compass-spinning');
            setTimeout(() => compassBtn.classList.remove('compass-spinning'), 800);
        }

        const data = HawdajData.get();
        if (activeMapMode === 'regions') {
            const uncompleted = Object.keys(data.regions).filter(id => !completedRegions.includes(id));
            if (uncompleted.length === 0) { HawdajApp.showToast('اكتشفت جميع المناطق الجغرافية!', 'info'); return; }
            const hintGroup = uncompleted[Math.floor(Math.random() * uncompleted.length)];
            flashGroupPaths(hintGroup);
            HawdajApp.showToast(`💡 تلميح: توجه إلى ${data.regions[hintGroup]?.name || hintGroup}`, 'success');
        } else {
            const uncompleted = Object.keys(HawdajData.CITIES_DATA).filter(id => !completedCities.includes(id));
            if (uncompleted.length === 0) { HawdajApp.showToast('استكشفت جميع المحافظات!', 'info'); return; }
            const hintCity = uncompleted[Math.floor(Math.random() * uncompleted.length)];
            const path = document.getElementById(`region-${hintCity}`);
            if (path) {
                path.style.transition = 'all 0.2s';
                path.style.fill = '#FFD700';
                path.style.filter = 'drop-shadow(0 0 10px rgba(255,215,0,0.8))';
                setTimeout(() => { path.style.fill = ''; path.style.filter = ''; path.style.transition = ''; }, 2000);
            }
            HawdajApp.showToast(`💡 تلميح: توجه إلى ${HawdajData.CITIES_DATA[hintCity]?.name || hintCity}`, 'success');
        }
        HawdajAudio.SFX.correct();
    }

    function flashGroupPaths(geoGroupId) {
        const container = document.getElementById('map-container');
        if (!container) return;
        const groupPaths = [];
        container.querySelectorAll('path[data-region]').forEach(p => {
            if (REGION_MAPPING[p.dataset.region] === geoGroupId) {
                groupPaths.push(p);
                p.style.transition = 'all 0.2s ease';
                p.style.fill = '#FFD700';
                p.style.filter = 'drop-shadow(0 0 10px rgba(255,215,0,0.8))';
            }
        });
        let t = 0;
        const ival = setInterval(() => {
            t++;
            groupPaths.forEach(gp => { gp.style.transform = t % 2 === 0 ? 'scale(1.02)' : 'scale(1)'; });
            if (t > 5) {
                clearInterval(ival);
                groupPaths.forEach(gp => { gp.style.fill = ''; gp.style.filter = ''; gp.style.transform = ''; gp.style.transition = ''; });
                selectRegionGroup(geoGroupId);
            }
        }, 300);
    }

    function setLastClicked(id) { lastClickedId = id; }
    function getLastClicked() { return lastClickedId; }

    return { init, renderMap, closeRegionModal, showRegionInfo, showCityInfo, setLastClicked, getLastClicked, useHint };
})();

window.MapGame = MapGame;
